...
function stableCapabilitySetVersion(
  ...
  const canonical = JSON.stringify({
    ...
    capabilities: capabilities.map((capability) => ({
      ...
      permissionCode: capability.permissionCode ?? null
    }))
  });
  ...
  async listCapabilities(
    ...
    for (const instance of instances) {
      ...
      for (const command of effective.commands) {
        const permissionCode = stringConfig(command.config, 'permissionCode');
        capabilities.push({
    ...
    return {
      ...
      capabilitySetVersion: stableCapabilitySetVersion(enterpriseId, capabilities),
      capabilities
    };
  }

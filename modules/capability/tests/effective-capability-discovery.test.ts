...
class FakeMetadataReader implements MetadataReader {
  constructor(private readonly instances: readonly ApplicationInstance[]) {}
  ...
  async getCommands(
    ...
    if (applicationDefinitionVersionId.startsWith('salesDef:')) {
      return [{
        ...
        config: { permissionCode: 'sales.approve' }
      }];
    }
...
describe('DefaultEffectiveCapabilityDiscovery', () => {
  it('exposes command capabilities only from effective active applications', async () => {
    ...
    const catalog = await discovery.listCapabilities('ent-1');

    expect(apps.applications.map((x) => x.applicationCode)).toEqual([
      'sales_order'
    ]);
    expect(catalog.capabilities.map((x) => x.code)).toEqual([
      'sales_order.approve-sales-order'
    ]);
    expect(catalog.capabilities[0]?.applicationInstanceId).toBe('sales-instance');
    expect(catalog.capabilities[0]?.permissionCode).toBe('sales.approve');
    expect(catalog.source).toBe('COMMAND_DEFINITION_BOOTSTRAP');
  });

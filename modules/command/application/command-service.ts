...
export class CommandService implements CommandExecutor {
  constructor(
    private readonly capabilities: CommandCapabilityResolver,
    private readonly transaction: CommandTransactionPort
  ) {}
  ...
  private validateRequest(request: ExecuteCommandRequest): void {
    if (request.idempotencyKey.trim().length === 0) {
      throw new AppError({
        ...
        message: 'Command idempotency key is required.',
        module: 'command',
        operation: 'execute'
      });
    }

    if (request.businessObjectKey.trim().length === 0) {
      throw new AppError({
        code: 'BUSINESS_OBJECT_KEY_REQUIRED',
        message: 'Business object key is required.',
        module: 'command',
        operation: 'execute'
      });
    }

    if (Number.isNaN(request.effectiveAt.getTime())) {
      throw new AppError({
        code: 'INVALID_EFFECTIVE_TIME',
        message: 'Command effective time is invalid.',
        module: 'command',
        operation: 'execute'
      });
    }

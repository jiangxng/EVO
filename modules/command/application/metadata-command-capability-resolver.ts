...
export class MetadataCommandCapabilityResolver
  implements CommandCapabilityResolver
{
  ...
  async resolve(
    ...
    const instance = await this.metadata.getApplicationInstance(
      enterpriseId,
    ...
    if (instance === null || instance.status !== 'ACTIVE') {
      throw new AppError({
        code: 'COMMAND_APPLICATION_NOT_AVAILABLE',
        message: 'Application instance is not available.',
        module: 'command',
        operation: 'resolveCommandCapability',
    ...
    const version =
      instance.pinnedDefinitionVersion === null
        ? await this.metadata.getPublishedApplicationDefinitionVersion(
            instance.applicationDefinitionId
          )
        : await this.metadata.getApplicationDefinitionVersion(
            instance.applicationDefinitionId,
    ...
    if (version === null || version.status !== 'PUBLISHED') {
      throw new AppError({
        code: 'COMMAND_METADATA_NOT_PUBLISHED',
        message: 'Command metadata is not available from a published definition.',
        module: 'command',
        operation: 'resolveCommandCapability',
        details: { applicationInstanceId, commandCode }
      });
    }
    ...
    if (command === undefined) {
      throw new AppError({
        ...
        message: 'Command is not defined for this application.',
        module: 'command',
        operation: 'resolveCommandCapability',
        details: { applicationInstanceId, commandCode }
      });

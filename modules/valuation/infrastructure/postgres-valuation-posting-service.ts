...
export class PostgresValuationPostingService implements ValuationPostingService {
  ...
  async postCostResult(costResultId: string): Promise<ValuationPostingResult> {
    return this.db.transaction().execute(async (trx) => {
      ...
      const existing = await trx.selectFrom('valuation_position').selectAll().where('enterprise_id','=',cost.enterprise_id).where('business_data_id','=',cost.business_data_id).where('valuation_rule_id','=',rule.id).forUpdate().executeTakeFirst();
      ...
      if (delta.isZero()) {
        if (existing === undefined) await trx.insertInto('valuation_position').values({enterprise_id:cost.enterprise_id,business_data_id:cost.business_data_id,valuation_rule_id:rule.id,valuation_rule_version:rule.version,total_cost:target.toString(),last_cost_result_id:cost.id}).execute();
      ...
      for(let index=0;index<effects.length;index+=1){
        ...
        await trx.insertInto('ledger_balance').values({enterprise_id:cost.enterprise_id,consistency_domain:postingInput.consistency_domain,ledger_dataset_id:datasetId,ledger_definition_id:ledger.id,dimension_hash:hash,dimensions,quantity:'0',amount:effect.amount.toString(),last_effective_at:business.effective_at,last_posting_priority:postingInput.posting_priority+1000000,last_posting_sequence:postingInput.posting_sequence}).onConflict((oc)=>oc.columns(['ledger_dataset_id','ledger_definition_id','dimension_hash']).doUpdateSet({amount:sql`ledger_balance.amount + excluded.amount`,last_effective_at:business.effective_at,last_posting_priority:postingInput.posting_priority+1000000,last_posting_sequence:postingInput.posting_sequence,updated_at:sql`now()`})).execute();
      }
      await trx.insertInto('valuation_position').values({enterprise_id:cost.enterprise_id,business_data_id:cost.business_data_id,valuation_rule_id:rule.id,valuation_rule_version:rule.version,total_cost:target.toString(),last_cost_result_id:cost.id}).onConflict((oc)=>oc.columns(['enterprise_id','business_data_id','valuation_rule_id']).doUpdateSet({valuation_rule_version:rule.version,total_cost:target.toString(),last_cost_result_id:cost.id,updated_at:sql`now()`})).execute();
  ...
  private async ensureActiveDataset(trx: DatabaseTransaction, enterpriseId: string, consistencyDomain: string): Promise<string> {
    const existing=await trx.selectFrom('ledger_dataset').select('id').where('enterprise_id','=',enterpriseId).where('consistency_domain','=',consistencyDomain).where('status','=','ACTIVE').executeTakeFirst();
    if(existing!==undefined)return existing.id;
    return (await trx.insertInto('ledger_dataset').values({enterprise_id:enterpriseId,consistency_domain:consistencyDomain,kind:'CURRENT',status:'ACTIVE',posting_boundary_sequence:null,activated_at:sql`now()`}).returning('id').executeTakeFirstOrThrow()).id;
  }
}

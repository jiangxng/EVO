...
4. Posting rule evaluation is deterministic for the same BusinessData and the same supplied rule/configuration input. Core does not own PostingRule version lifecycle.
...
14. Ledger records retain PostingInput, BusinessData, supplied PostingRule identity/diagnostic lineage and PostingRun lineage; such lineage does not create a Core-managed PostingRule version model.

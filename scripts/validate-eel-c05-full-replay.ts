...
import { computeEconomicRuntimeDigest, computeReplayInputDigest } from '../modules/replay/infrastructure/postgres-replay-digest.js';
...
async function transferSnapshot(
  ...
  factIds:string[],
  receiptFactIds:string[]
){
  const source=await inventoryByWarehouse(enterpriseId,productId,sourceWarehouse);
  const destination=await inventoryByWarehouse(enterpriseId,productId,destinationWarehouse);
  const pending=await pendingTransfer(enterpriseId,transferNo);
  const work=await transferWork(enterpriseId,transferNo);
  const facts=await runtime.db.selectFrom('business_data')
    ...
    .where('enterprise_id','=',enterpriseId)
    .where('id','in',factIds)
    .orderBy('effective_at').orderBy('id').execute();
  const links=(await runtime.db.selectFrom('business_object_link')
    ...
    .sort((a,b)=>a.from_business_data_id.localeCompare(b.from_business_data_id)||
      a.to_business_data_id.localeCompare(b.to_business_data_id)||
      a.relation_type.localeCompare(b.relation_type));
  const costs=[];
  for(const factId of receiptFactIds){
    const row=await runtime.db.selectFrom('cost_result as c')
      .innerJoin('cost_run as r','r.id','c.cost_run_id')
      .select(['c.business_data_id','c.quantity','c.unit_cost','c.total_cost'])
      .where('r.enterprise_id','=',enterpriseId)
      .where('c.business_data_id','=',factId)
      .orderBy('r.started_at','desc').executeTakeFirstOrThrow();
    costs.push(row);
  }
  return {
    source:{quantity:source.quantity.toString(),amount:source.amount.toString()},
    destination:{quantity:destination.quantity.toString(),amount:destination.amount.toString()},
    pending:pending===undefined?null:{quantity:pending.quantity,amount:pending.amount},
    work:work===undefined?null:{type:work.work_type,status:work.status,quantity:work.source_quantity},
    facts,links,costs
  };
}
...
try{
  ...
  const factIds=[poFact.id,receipt.businessDataId,createdFact.id,issuedFact.id,...receiptFactIds];
  const before=await transferSnapshot(
    ids.enterpriseId,transferNo,productId,sourceWarehouse,destinationWarehouse,factIds,receiptFactIds
  );
  ...
  const beforeInput=await computeReplayInputDigest(runtime.db,ids.enterpriseId,runtimeState.consistency_domain,boundary);
  const beforeEconomic=await computeEconomicRuntimeDigest(runtime.db,ids.enterpriseId,runtimeState.consistency_domain,boundary);
  ...
  const after=await transferSnapshot(
    ids.enterpriseId,transferNo,productId,sourceWarehouse,destinationWarehouse,factIds,receiptFactIds
  );
  const afterInput=await computeReplayInputDigest(runtime.db,ids.enterpriseId,runtimeState.consistency_domain,boundary);
  const afterEconomic=await computeEconomicRuntimeDigest(runtime.db,ids.enterpriseId,runtimeState.consistency_domain,boundary);
  ...
  if(before.pending===null||!new Decimal(before.pending.quantity).eq(0)||before.work?.status!=='DONE'){
    throw new Error(`Transfer must be closed before and after replay: ${JSON.stringify(before)}`);
  }
  if(before.costs.length!==2||before.costs[0]?.total_cost===null||before.costs[1]?.total_cost===null||
    !new Decimal(before.costs[0]!.total_cost!).eq(150)||!new Decimal(before.costs[1]!.total_cost!).eq(250)){
    throw new Error(`Transfer FIFO costs must remain 150 + 250: ${JSON.stringify(before.costs)}`);
  }
  ...
  console.log(JSON.stringify({
    status:'PASS',
    packet:'EEL-C05-FULL-REPLAY',
    transferNo,
    recognitionPolicy:'DESTINATION_RECEIPT',
    final:before,
    canonicalHistoryPreserved:true,
    explicitRelationshipsPreserved:true,
    inventoryQuantityConserved:true,
    inventoryValueConserved:true,
    transferCostsRebuiltIdentically:true,

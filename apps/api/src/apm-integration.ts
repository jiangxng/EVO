...
const APM = {
  ...
  product: 'Critical Servo Module', materialId: 'SERVO-MODULE-CRITICAL', promiseDate: '2026-09-18', revenueAtRisk: 280000,
...
export async function apmObservation(runtime: Runtime) {
  ...
  return {
    contractVersion:'1.0.0', observationId, deliveryId:`delivery:${observationId}:${observedAt}`, enterpriseId:APM.enterpriseId,
...
export function registerApmIntegration(app:FastifyInstance,runtime:Runtime):void {
  ...
  app.post('/api/v1/apm/command-proposals/admit',async(request)=>{
    ...
    return {admitted:true,executionStatus:'NOT_EXECUTED',proposalId:body.proposalId,commandCode:body.commandCode,proposedInput,correlationId:body.correlationId,authorization:'EVO_REEVALUATES_AT_EXECUTION'};
  });
  app.post('/api/v1/apm/actions/execute',async(request)=>{
    ...
    const confirmation=body.confirmationEvidence as Record<string,unknown>|undefined;
    if(confirmation?.confirmed!==true) fail('HUMAN_CONFIRMATION_REQUIRED','Human confirmation evidence is required.');
    const values=body.submittedValues as Record<string,unknown>|undefined;

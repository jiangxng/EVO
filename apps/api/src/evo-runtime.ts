...
export async function demoIds(runtime: ReturnType<typeof createEvoRuntime>) {
  ...
  return { enterpriseId:enterprise.id, salesAppId:apps.find(x=>x.code==='sales')?.id??'', productionDemandAppId:apps.find(x=>x.code==='production-demand')?.id??'', materialIssueAppId:apps.find(x=>x.code==='material-issue')?.id??'', productionAppId:apps.find(x=>x.code==='production')?.id??'', inventoryAppId:apps.find(x=>x.code==='inventory')?.id??'', inventoryTransferAppId:apps.find(x=>x.code==='inventory-transfer')?.id??'', valuationAppId:apps.find(x=>x.code==='valuation')?.id??'', cashAppId:apps.find(x=>x.code==='cash')?.id??'', cashPaymentAppId:apps.find(x=>x.code==='cash-payment')?.id??'', cashRefundAppId:apps.find(x=>x.code==='cash-refund')?.id??'', salesReturnAppId:apps.find(x=>x.code==='sales-return')?.id??'', salesExchangeAppId:apps.find(x=>x.code==='sales-exchange')?.id??'', salesInvoiceAppId:apps.find(x=>x.code==='sales-invoice')?.id??'', procurementAppId:apps.find(x=>x.code==='procurement')?.id??'', flowDefinitionId:(await runtime.db.selectFrom('flow_definition').select('id').where('enterprise_id','=',enterprise.id).where('code','=','order-to-cash').where('version','=',1).executeTakeFirstOrThrow()).id, procureToPayFlowDefinitionId:(await runtime.db.selectFrom('flow_definition').select('id').where('enterprise_id','=',enterprise.id).where('code','=','procure-to-pay').where('version','=',1).executeTakeFirstOrThrow()).id, manufacturingFlowDefinitionId:(await runtime.db.selectFrom('flow_definition').select('id').where('enterprise_id','=',enterprise.id).where('code','=','manufacturing-execution').where('version','=',1).executeTakeFirstOrThrow()).id, inventoryTransferFlowDefinitionId:(await runtime.db.selectFrom('flow_definition').select('id').where('enterprise_id','=',enterprise.id).where('code','=','inventory-transfer').where('version','=',1).executeTakeFirstOrThrow()).id };
}
